...
# Custom modules
import profiler
from file_params import INPUT_FILES, RESULTS_PATH
...
def traitement(baseline_files, files, animals, cells, scaled=True):
    ...
    sample_BL = FCMeasurement(ID='Test Sample', datafile=os.path.join(INPUT_FILES, baseline_files[0])).data
    ...
    for file in baseline_files:
        ...
        sample = FCMeasurement(ID='Test Sample', datafile=os.path.join(INPUT_FILES, file)).data
        # sample = sample.data
        for animal in animals:
            if animal in file:
    ...
    indexes = random.sample(range(len(sample_BL)), cells)
    data_BL = sample_BL
    del sample_BL
    data_BL = data_BL.iloc[indexes]

    columns_to_drop = ['Time', 'Eventlength', 'Center', 'Offset', 'Width',
    ...
    for match in matches:
        ...
        key = match
        variables[key] = FCMeasurement(ID='Test Sample', datafile=os.path.join(INPUT_FILES, timepoint_files[0])).data
