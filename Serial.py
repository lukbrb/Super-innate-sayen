...
from utils import unimodal_bool, spread_bool, create_files


def UMAP_clusters(directory, cells, neighbors, metric):
    # Quality control tests used in QC section

    baseline_files, files = create_files(metric)
    ...
    for file in baseline_files:
        sample = FCMeasurement(ID='Test Sample', datafile=f"files/{file}").data
        # sample = sample.data
